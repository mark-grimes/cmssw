...
namespace l1menu
{
	/** @brief A singleton that can create ITriggerAlgorithm instances of a given type and version.
	 *
	 * This is a global store for all available triggers. Code in each implementation of ITriggerAlgorithm
	...
	public:
		...
		struct TriggerVersion
		{
			...
			bool operator==( const TriggerVersion& otherTriggerVersion ) const;
		};
	public:
		...
		std::unique_ptr<l1menu::ITriggerAlgorithm> getTrigger( const std::string& name ) const;

		/** @brief Get a specific version of the trigger with the supplied name. */
		std::unique_ptr<l1menu::ITriggerAlgorithm> getTrigger( const std::string& name, unsigned int version ) const;
		std::unique_ptr<l1menu::ITriggerAlgorithm> getTrigger( const TriggerVersion& details ) const;

		/** @brief Provides a copy of the supplied trigger, with the correct version and also copyies the parameters. */
		std::unique_ptr<l1menu::ITriggerAlgorithm> copyTrigger( const l1menu::ITrigger& triggerToCopy ) const;

		/** @brief List the triggers available.
		 *
		 * Returns a vector of the TriggerTable::TriggerVersion convenience class.
		 */
		std::vector<l1menu::TriggerTable::TriggerVersion> listTriggers() const;
		 ...
		 */
		void registerTrigger( const std::string& name, unsigned int version, std::unique_ptr<l1menu::ITriggerAlgorithm> (*creationFunctionPointer)() );
