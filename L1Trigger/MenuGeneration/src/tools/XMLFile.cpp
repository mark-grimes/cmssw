...
namespace l1menu
{
	namespace tools
	{
			...
			: pOwnedDocument_(nullptr,[](xercesc::DOMDocument* p){if(p!=nullptr){p->release();}})
		{
			...
			if( pDomImplementation_==nullptr ) throw l1menu::Exception( "Couldn't create the DOMImplementation" );
...
void l1menu::tools::XMLFile::parseFromFile( const std::string& filename )
{
	...
	if( pImple->pDocument_==nullptr )
	{
		...
		throw l1menu::IncorrectFileFormatException( "Couldn't open the file "+filename );
	}
	else if( pImple->pDocument_->getDocumentElement()==nullptr ) throw l1menu::IncorrectFileFormatException( filename+" doesn't appear to be an xml file" );
