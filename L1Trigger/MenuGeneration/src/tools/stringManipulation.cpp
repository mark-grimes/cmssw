...
float l1menu::tools::convertStringToFloat( const std::string& string )
{
	...
	if( stringConverter.fail() || !stringConverter.eof() ) throw l1menu::InvalidArgumentException( "Unable to convert \""+string+"\" to a float" );
	return returnValue;
}
...
int l1menu::tools::convertStringToInt( const std::string& string )
{
	...
	std::stringstream stringConverter( string );
	stringConverter >> returnValue;
	if( stringConverter.fail() || !stringConverter.eof() ) throw l1menu::InvalidArgumentException( "Unable to convert \""+string+"\" to an int" );
	return returnValue;
}
