...
void TriggerTableUnitTestSuite::testGettingAndSettingAllTriggerParameters()
{
	...
	const auto& allTriggerVersions=table.listTriggers();

	// Loop over all of the triggers
	for( const auto& triggerDetails : allTriggerVersions )
	{
		std::unique_ptr<l1menu::ITriggerAlgorithm> pTrigger=table.getTrigger( triggerDetails.name, triggerDetails.version );
...
void TriggerTableUnitTestSuite::dumpTriggerTable()
{
	...
	std::cout << "\n"
			...
			<< "--------------------------------" << std::endl;
	std::vector<l1menu::TriggerTable::TriggerVersion> listOfTriggers=triggerTable.listTriggers();
	for( std::vector<l1menu::TriggerTable::TriggerVersion>::const_iterator iTriggerEntry=listOfTriggers.begin(); iTriggerEntry!=listOfTriggers.end(); ++iTriggerEntry )
	{
